...
/**
 * This class is the inventory of the virtual store and contains the information about the store's stock which
 * implements the ProductStockContainer class
 */
public class Inventory implements ProductStockContainer {
     ...
     * @param product Product, ID of the desired product
     * @return int, returns the amount of stock available in inventory for the specified product
     */
    @Override
    public int getProductQuantity(Product product){                     //added in milestone 1
        for (int i = 0; i < this.product.size(); i++){
            if(product.getID() == this.product.get(i).getID()){
     ...
     */
    @Override
    public void addProductQuantity(Product newProduct, int numStock){     //added in milestone 1, edited in milestone 3
     ...
     * @param product Product, type of specified product
     ...
     */
    @Override
    public boolean removeProductQuantity(Product product, int stockRemoved){    //added in milestone 1
        int productIndex = -1;  //ensures productIndex is invalid index so no stock is added to the wrong product
        ...
        if(productIndex != -1){  //if product is in inventory
    ...
    /**
     * Method that gets the number of unique products in the inventory
